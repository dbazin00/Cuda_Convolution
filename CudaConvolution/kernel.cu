...
__global__
void blur(unsigned char* input_image, unsigned char* output_image, int width, int height, int filterSize) {

    const unsigned int offset = blockIdx.x * blockDim.x + threadIdx.x;
    int x = offset % width;
    int y = (offset - x) / width;
    int fsize = filterSize; // Filter size
...
void blurFilter(unsigned char* input_image, unsigned char* output_image, int width, int height, int fsize) {
    ...
    blur <<<gridDims, blockDims>>> (dev_input, dev_output, width, height, fsize);
